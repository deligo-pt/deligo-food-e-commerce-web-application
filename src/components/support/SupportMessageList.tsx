...
export default function SupportMessageList({
  ...
  if (isError) {
    return (
      <Centered>
        <p className="text-sm font-medium text-[#191c1d] dark:text-neutral-50">
          {t("somethingWentWrong")}
        </p>
        <button
          type="button"
          onClick={() => refetch()}
          className="mt-3 cursor-pointer rounded-lg bg-[#f9186b] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#d4145b] dark:bg-pink-600 dark:hover:bg-pink-700"
        >
          {t("tryAgain")}
        </button>
      </Centered>
  ...
  if (messages.length === 0) {
    return (
      <Centered>
        ...
        <p className="mt-1 max-w-[16rem] text-sm text-[#5a4044] dark:text-neutral-400">
          {t("supportEmptyHint")}
        </p>
      </Centered>
    );
      ...
      {hasOlder && (
        <div className="mb-4 flex justify-center">
          <button
            ...
            className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-1.5 text-xs font-semibold text-[#f9186b] transition-colors hover:bg-pink-50 disabled:cursor-default disabled:opacity-60 dark:border-neutral-800 dark:bg-neutral-900 dark:text-pink-400 dark:hover:bg-pink-950/30"
          >
      ...
      {days.map((day) => (
        <section key={day.key} className="mb-4 last:mb-0">
          ...
          <div className="relative my-4 flex items-center justify-center">
            ...
            />
            <span className="relative rounded-full border border-gray-200 bg-[#f8f9fa] px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-[#5a4044] dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-400">
              {day.label}
            </span>
          </div>
